...
class SoundManager {
  ...
  static void playWrongSound() async {
    await _playLocalSound('sounds/incorrect.wav');
  }

  static void playCorrectSound() async {
    await _playLocalSound('sounds/correct.wav');
  }

  static void playFinishGameSound() async {
    await _playLocalSound('sounds/congrat.wav');
  }

  static void playTetrisStart(bool isMuted) async {
    if (isMuted) return;
    await _playLocalSound('sounds/start.mp3');
  }

  static void playTetrisClear(bool isMuted) async {
    if (isMuted) return;
    await _playLocalSound('sounds/clean.mp3');
  }

  static void playTetrisFall(bool isMuted) async {
    if (isMuted) return;
    await _playLocalSound('sounds/drop.mp3');
  }

  static void playTetrisRotate(bool isMuted) async {
    if (isMuted) return;
    await _playLocalSound('sounds/rotate.mp3');
  }

  static void playTetrisMove(bool isMuted) async {
    if (isMuted) return;
    await _playLocalSound('sounds/move.mp3');
  }

  static Future _playLocalSound(String sound) async {
